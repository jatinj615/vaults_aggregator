...
export const getBalances = ({ _erc20, dataPoolStats, dataEpoches, kovanEpochData, kovanPoolData, signer }) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    dataEpoches?.epoches.forEach((epoch) => {
    ...
    forEach(dataEpoches?.epoches, (epoch) => {
      ...
      getApyPromises.push(getApy(epoch?.address, signer));
    });
    ...
    try {
      ...
      const getApyResults = await Promise.all(getApyPromises);
      forEach(calculatedBalanceResults, (ytBalance, index) => {
        ...
        if (ytBalance && !ytBalance.isZero()) {
          ...
          ytBalanceObject.vaultApy = getApyResults[index];
